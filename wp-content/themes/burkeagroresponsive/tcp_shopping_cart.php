...
if ( $source->has_order_details() ) :
	...
	<tr class="tcp_cart_product_row <?php if ( $i++ & 1 == 1 ) : ?> par<?php endif; ?>">
		...
		<?php if ( $source->see_weight() ) : ?>
			<td class="tcp_cart_weight"><?php echo str_replace('.00','',tcp_number_format( $order_detail->get_weight() )); ?>&nbsp;<?php echo tcp_get_the_unit_weight(); ?></td>
		<?php endif; ?>
...
<?php
	...
	if ( $source->see_tax_summary() && $total_tax > 0 ) 
	{
		...
		if ($total < 1000)
		{
			$a_pagar = $total + $total_tax;
			$applied_tax = $total_tax;
			$total_tax = 0;
			$total_text = 'Total con retención aplicada';
		}
		else
		{
		$a_pagar = $total;
		$total += $total_tax;
		$applied_tax = $total_tax;
		$total_text = 'Total antes de retención';
		}
	}
	else
	{
		$total_tax = ($total > 999) ? $total_tax : 0;
		$a_pagar = ($total > 999) ? $total - $total_tax : $total + $total_tax;
		$applied_tax = $total_tax;
		$total_text = ($total > 999) ? 'Total sin retención aplicada' : 'Total antes de retención';
...
?>
	...
	<tr class="tcp_cart_subtotal_row">
		<td colspan="<?php echo $colspan; ?>" class="tcp_cart_subtotal_title"><?php _e( 'Monto a pagar', 'tcp' ); ?></td>
		<td class="tcp_cart_subtotal"><?php echo tcp_format_the_price( $a_pagar); ?></td>
	</tr>
		<tr class="tcp_cart_tax_row">
			<td colspan="<?php echo $colspan;?>" class="tcp_cart_tax_title">Retención</td>
			<td class="tcp_cart_tax"><?php echo tcp_format_the_price( $applied_tax ); ?></td>
		</tr>
	<tr class="tcp_cart_total_row">
		<td colspan="<?php echo $colspan; ?>" class="tcp_cart_total_title"><?php _e( $total_text, 'tcp' ); ?></td>
